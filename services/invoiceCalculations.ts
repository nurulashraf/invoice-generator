...
/** Tax amount for the given percentage rate (e.g. 6 => 6%). */
export const taxAmount = (items: LineItem[], taxRate: number): number =>
  subtotal(items) * ((Number(taxRate) || 0) / 100);

/** Grand total including tax. */
export const grandTotal = (items: LineItem[], taxRate: number): number =>
